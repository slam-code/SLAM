...
TEST(SearchParameters, Construction) {
  ...
  EXPECT_EQ(11, search_parameters.num_scans);             //2*n+1
  EXPECT_EQ(11, search_parameters.linear_bounds.size());  //2*n+1
...
TEST(Candidate, Construction) {
  ...
  EXPECT_NEAR(0.25, candidate.x, 1e-9);// x(-y_index_offset * search_parameters.resolution),5*0.05
  EXPECT_NEAR(-0.2, candidate.y, 1e-9);// y(-x_index_offset * search_parameters.resolution),-4*0.05
/*
