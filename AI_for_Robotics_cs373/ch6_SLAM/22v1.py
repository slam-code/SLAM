...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    dim=2*(N+num_landmarks) #x,y各占一行。

    Omgea =matrix()
    Omgea.zero(dim,dim)
    Omgea.value[0][0]=1.0 #x0为word的中心
    Omgea.value[1][1]=1.0 #y0也是中心

    Xi=matrix()
    Xi.zero(dim,1)        
    Xi.value[0][0]=world_size/2.0 #中心
    Xi.value[1][0]=world_size/2.0 #中心

    print "data[][]:"
    print data[0][0],"--\n"
    print data[0][1]

    for k in range(len(data)): # 对所有的移动和测量data
        n=k*2

        measurement=data[k][0] #测量。5个相对于landmark的距离。
        motion=data[k][1]      #移动。

        for i in range(len(measurement)):#更新测量后的结果
            m=2*(N+measurement[i][0])

            for b in range(2):
                Omgea.value[n+b][n+b]+=1.0/measurement_noise
                Omgea.value[m+b][m+b]+=1.0/measurement_noise
                Omgea.value[n+b][m+b]+=-1.0/measurement_noise
                Omgea.value[m+b][n+b]+=-1.0/measurement_noise
                Xi.value[n+b][0]+=-measurement[i][1+b]/measurement_noise
                Xi.value[m+b][0]+=measurement[i][1+b]/measurement_noise


        for b in range(4):
            Omgea.value[n+b][n+b]+=1.0/motion_noise


        for b in range(2):
            Omgea.value[n+b][n+b+2]+=-1.0/motion_noise
            Omgea.value[n+b+2][n+b]+=-1.0/motion_noise
