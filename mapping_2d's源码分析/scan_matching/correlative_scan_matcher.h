...
/*
SearchParameters类定义了搜索空间参数信息

数据成员
1,resolution 分辨率
2,num_angular_perturbations 角度扰动
3,num_scans扫描数
4,angular_perturbation_step_size每一步的扰动
*/
// Describes the search space.
struct SearchParameters {
  // Linear search window in pixel offsets; bounds are inclusive.
  struct LinearBounds { //搜索方框
    int min_x;
    int max_x;
    int min_y;
    int max_y;
  ...
  // Tightens the search window as much as possible.减小,收窄
...
struct Candidate {
  ...
  // Index into the rotated scans vector.旋转扫描向量
  int scan_index = 0;

  // Linear offset from the initial pose.相对于 初始位姿的 线性偏移
  int x_index_offset = 0;
  int y_index_offset = 0;

  // Pose of this Candidate relative to the initial pose.x,y是相对于初始位置的偏移量
  double x = 0.;
  double y = 0.;
  double orientation = 0.;

  // Score, higher is better. 匹配分数,越高越匹配
  float score = 0.f;
