...
constexpr FloatType sqr(FloatType a) {//平方,常量表达式
  return a * a;
}
...
Eigen::Matrix<FloatType, N, N> MatrixSqrt(
  ...
  Eigen::SelfAdjointEigenSolver<Eigen::Matrix<FloatType, N, N>>
      adjoint_eigen_solver((A + A.transpose()) / 2.);//构造函数
...
*/
  ...
  void Predict(std::function<StateType(const StateType&)> g,
    ...
    std::vector<StateType> Y;//N*1矩阵
    Y.reserve(2 * N + 1);//p65
    Y.emplace_back(g(mu));//状态转移,3.68

    const FloatType kSqrtNPlusLambda = std::sqrt(N + kLambda);//p65,3.66
    for (int i = 0; i < N; ++i) {
...
*/
 ...
 private:
  ...
  GaussianDistribution<FloatType, N> belief_; //N*1矩阵
