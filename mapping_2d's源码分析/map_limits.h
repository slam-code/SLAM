...
class MapLimits {
 public:
  ...
  // the length of one side.按公制计,分辨率即一个网格的大小
  double resolution() const { return resolution_; }

  // Returns the corner of the limits, i.e., all pixels have positions with
  // smaller coordinates.(x,y)均小于max_
  const Eigen::Vector2d& max() const { return max_; }

  // Returns the limits of the grid in number of cells.网格数量
  ...
  // false for Contains().获取(x,y)对应的网格的坐标
  ...
  // Returns true of the ProbabilityGrid contains 'xy_index'.返回xy是否在网格中
