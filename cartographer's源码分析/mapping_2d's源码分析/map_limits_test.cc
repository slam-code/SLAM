...
TEST(MapLimitsTest, ToProto) {
  const MapLimits limits(42., Eigen::Vector2d(3., 0.), CellLimits(2, 3));//分辨率,最大边界,网格数
...
TEST(MapLimitsTest, ConstructAndGet) {
  ...
  EXPECT_EQ(2, cell_limits.num_x_cells);              // x方向是2个
  EXPECT_EQ(3, cell_limits.num_y_cells);              // y是3个

  const Eigen::Vector2d& max = limits.max(); //x最大值是3,y最大值是2 
  EXPECT_EQ(3., max.x());
  EXPECT_EQ(0., max.y());
