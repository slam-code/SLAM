...
根据 options选项有选择的添加topics
*/
int Node::AddTrajectory(const TrajectoryOptions& options,
                        const cartographer_ros_msgs::SensorTopics& topics) {
  std::unordered_set<string> expected_sensor_ids;

  if (options.use_laser_scan) {                         //轨迹线使用激光扫描器
    expected_sensor_ids.insert(topics.laser_scan_topic);//添加一个激光的topic
  }
  ...
  if (options.trajectory_builder_options.trajectory_builder_2d_options()
          .use_imu_data()) {
    expected_sensor_ids.insert(topics.imu_topic);       //添加imu的topic
  }
  if (options.use_odometry) {
    expected_sensor_ids.insert(topics.odometry_topic); //添加里程计的topic
  }
  return map_builder_bridge_.AddTrajectory(expected_sensor_ids, options); //添加传感器ID
}

void Node::LaunchSubscribers(const TrajectoryOptions& options,
                             const cartographer_ros_msgs::SensorTopics& topics,
                             const int trajectory_id) {
  if (options.use_laser_scan) {//使用激光扫描
...
void Node::StartTrajectoryWithDefaultTopics(const TrajectoryOptions& options) {
  ...
  const int trajectory_id = AddTrajectory(options, topics);  //根据options和topics添加轨迹线
