...
Eigen::Matrix<double, 1, 1> Scalar(double value) {//按val倍返回单位矩阵
  return value * Eigen::Matrix<double, 1, 1>::Identity();
}

// A simple model that copies the first to the second state variable.
Eigen::Matrix<double, 2, 1> g(const Eigen::Matrix<double, 2, 1>& state) {//值拷贝,返回含相同内容的对象
...
TEST(KalmanFilterTest, testConstructor) {
  ...
  */
  ...
  EXPECT_NEAR(42., filter.GetBelief().GetMean()[1], 1e-9);//均值第1列是42,
}
